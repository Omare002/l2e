      ...
      <div className="mt-4 overflow-hidden rounded-lg border border-border">
        {query.isLoading && results.length === 0 ? (
          ...
          results.map((b) => (
            <div
              key={b.id}
              className="flex flex-wrap items-center gap-3 border-b border-border px-4 py-4 transition-colors duration-200 last:border-b-0 hover:bg-muted/50 sm:flex-nowrap sm:gap-4 sm:px-5"
            >
              ...
              <span className="shrink-0 text-right font-mono text-[12px] tabular-nums text-muted-foreground">
              ...
              <FollowButton targetId={b.id} username={b.username} className="shrink-0" />
            </div>
          ))
        )}
      </div>
