...
export function RaceTrack({
  ...
  if (racers.length === 0) {
    return (
      <div className="surface-card px-6 py-10 text-center text-[13px] text-muted-foreground">
          ...
          {racers.map((r, i) => {
            ...
            return (
              ...
              >
                <div className="flex w-[52%] shrink-0 items-center gap-2.5 sm:w-[26%] sm:gap-3">
                  <span
                    className={`w-4 shrink-0 text-right font-mono text-[11px] tabular-nums ${
                      isLeader ? "text-neon" : "text-muted-foreground"
                    }`}
                  >
                    {i + 1}
                  </span>
                ...
                <div className="shrink-0 font-mono text-[12px] tabular-nums text-foreground/75 sm:hidden">
                  {r.votes.toLocaleString()}
                </div>
              </motion.div>
            );
